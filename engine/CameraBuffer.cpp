...
using namespace akari::camera;
using namespace akari::window;

CameraBuffer::CameraBuffer() {
  VkDeviceSize bufferSize = sizeof(Camera);

  const uint32_t framesInFlight = Application::GetMaxFramesInFlight();
  ...
  for (size_t i = 0; i < framesInFlight; i++) {
    ...
    vmaCreateBuffer(Application::GetVmaAllocator(), &bufferInfo, &allocCreateInfo, &m_Buffers[i], &m_Allocations[i], &allocInfo);
...
CameraBuffer::~CameraBuffer() {
  VmaAllocator   allocator      = Application::GetVmaAllocator();
  const uint32_t framesInFlight = Application::GetMaxFramesInFlight();
...
void CameraBuffer::Render(PerspectiveCamera* camera) {
  ...
  memcpy(m_Mapped[Application::GetCurrentFrameIndex()], &data, sizeof(Camera));
}
