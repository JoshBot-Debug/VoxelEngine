...
template <typename T>
concept DataWithoutRCU = requires(T t) {
  requires std::same_as<decltype(t.Id), uint32_t>;
};

template <typename N, typename F>
concept FilterCallbackWithoutRCU = requires(F f, const N* node) {
 ...
 */
template <DataWithoutRCU T, uint8_t S = 64>
class SparseOctreeWithoutRCU {
...
private:
   ...
   * @param size      The size of the region represented by this node.
   */
  Node* SetWithoutCopy(Node* node, uint8_t x, uint8_t y, uint8_t z, T* data, uint8_t size) {
    if (size == 1) {
      node->Data = data;
      return node;
    }
   ...
   */
  template <typename F>
    requires FilterCallbackWithoutRCU<Node, F>
...
public:
   ...
   */
  SparseOctreeWithoutRCU() = default;
  ...
  void Set(uint8_t x, uint8_t y, uint8_t z, T* data) {
    m_Root = Set(m_Root, x, y, z, data, SIZE);
    m_Mask.Set(x, y, z);
  };
   ...
   * @param data      The data to insert.
   */
  void SetWithCopyWithAtomicRoot(uint8_t x, uint8_t y, uint8_t z, T* data) {
   ...
   */
  uint64_t (&GetAxisX(uint32_t nodeId))[SIZE * SIZE] {
  ...
  uint64_t (&GetAxisY(uint32_t nodeId))[SIZE * SIZE] {
    static thread_local uint64_t masks[SIZE * SIZE];
    std::memset(masks, 0, sizeof(masks));
  ...
  uint64_t (&GetAxisZ(uint32_t nodeId))[SIZE * SIZE] {
    static thread_local uint64_t masks[SIZE * SIZE];
    std::memset(masks, 0, sizeof(masks));
  ...
  void Clear(uint8_t x, uint8_t y, uint8_t z) {
    m_Root = Clear(m_Root, x, y, z, SIZE);
    m_Mask.Clear(x, y, z);
  };
  ...
  void Flatten(std::vector<FlatNode>& out) {
    ...
    if (m_Root->Data)
      out[index].SetId(m_Root->Data->Id);
  ...
  void Filter(std::vector<FilterNode>& out, F&& filter) {
    Filter(out, filter, {0, 0, 0}, SIZE, m_Root);
  };
