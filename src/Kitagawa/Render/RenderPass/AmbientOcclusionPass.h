...
#include "Kitagawa/Render/RenderPass/AmbientOcclusion/SSAOPass.h"
...
class AmbientOcclusionPass : public IPipeline {
...
public:
  ...
  void SelectOption(Type type) {
    switch (type) {
    case Type::SSAO:
      m_Pipeline = std::make_unique<SSAOPass>();
      break;

    default:
      throw std::runtime_error("Invalid ambient occlusion choice.");
      break;
    }
  }
