...
SSRPass::SSRPass() : m_Device(Akari::Application::GetDevice()) {}
...
void SSRPass::Initialize(const std::any &info) {
  m_Init = std::any_cast<SSRPassInit>(info);
}

void SSRPass::CreateBuffers() {
  // Metadata
  {
...
void SSRPass::GetDescriptorPoolSize(std::vector<VkDescriptorPoolSize> &pool) {
...
void SSRPass::CreateDescriptorSets(VkDescriptorPool descriptorPool) {
  ...
  // Descriptor Set 0
  {
    ...
    std::vector<VkWriteDescriptorSet> writes{
        ...
        VkWriteDescriptorSet{
            .sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
            .dstSet = m_DescriptorSets[0][0],
            .dstBinding = Binding::T_REFLECTION,
            .dstArrayElement = 0,
            .descriptorCount = 1,
            .descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,
            .pImageInfo = &ssrInfo,
...
void SSRPass::CreatePipeline() {
  ...
  VkShaderModule shaderModule = CreateShaderModule(
      getExecutableDir() + "/../src/Shaders/Pipeline/ssr.comp.spv");
...
bool SSRPass::OnResize(uint32_t width, uint32_t height) {
  return m_SSR->Resize(width, height);
}

void SSRPass::Render(VkCommandBuffer commandBuffer) {

  m_Metadata.frame = Akari::Application::GetFrameCount(),
  m_Metadata.worldSize = m_Init.world->GetSVO()->GetSize(),
  m_Metadata.maxSteps = 64;
  m_Metadata.stepSize = 0.1f;
  m_Metadata.epsilon = 0.1f;
  ...
  uint32_t groupCountX = (m_SSR->GetWidth() + groupSizeX - 1) / groupSizeX;
  uint32_t groupCountY = (m_SSR->GetHeight() + groupSizeY - 1) / groupSizeY;
  ...
  m_SSR->Transition(commandBuffer, VK_IMAGE_LAYOUT_GENERAL,
