...
void LightingPass::GetDescriptorPoolSize(
...
void LightingPass::CreateDescriptorSets(VkDescriptorPool descriptorPool) {
...
bool LightingPass::OnResize(uint32_t width, uint32_t height) {
  return m_DirectLight->Resize(width, height);
}
...
void LightingPass::Render(VkCommandBuffer commandBuffer) {
  ...
  uint32_t groupCountX =
      (m_DirectLight->GetWidth() + groupSizeX - 1) / groupSizeX;
